...
async function getFirebaseMessaging() {
  if (cachedMessaging === null) {
    cachedMessaging = await initializeFirebaseMessaging();
  }
...
async function initializeFirebaseMessaging() {
  try {
    ...
    if (!messaging) {
      throw new Error("Missing firebase messaging!");
    }
...
self.addEventListener("install", event => {
  event.waitUntil(Promise.allSettled([precache(), getFirebaseMessaging()]));
});
...
function createNotificationOptions(payload) {
  ...
  const options = {
    ...
    vibrate: Array.isArray(data.vibrate) ? data.vibrate : VIBRATION_PATTERN,
  ...
  try {
    ...
    for (const client of clientList) {
      if (client.url.includes(self.location.origin)) {
        await client.navigate(normalizedUrl).catch(() => {});
        await client.focus();
        return;
      }
    }
    ...
    console.error("Failed to open/focus client:", error);
  }
}
